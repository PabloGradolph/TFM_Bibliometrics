...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            df = pd.read_csv("analysis/data/datasets/departamentos_todos_autores.csv")
        ...
        for _, row in df.iterrows():
            id = row['author_id']
            community = row['departamento']

            author = Author.objects.filter(gesbib_id__iexact=id).first()
            if author:
                author.department_global = community
                author.save()
                count += 1
                self.stdout.write(self.style.SUCCESS(f"{author.gesbib_id} → departamento: {community}"))
            else:
                not_found.append(id)
